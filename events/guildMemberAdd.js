...
module.exports = {
  execute: async(client, member, db) => {
    ...
    let timeout_event = await db.fetch(`eventTimeout_0`)
    
    let events = await db.fetch(`event_0`)
    
    if (events !== null && timeout_event - (Date.now() - events) > 999) multiplier = 1
    
    data.logs.unshift(`[+${1 + multiplier}] - Sunucuya Katıldın!`)
    ...
    db.add(`coins_${member.user.id}`, 1 + multiplier)
